...
export default function TechStack() {
  return (
    <section className="motion-rise-in motion-delay-4 mt-16 rounded-[2rem] border border-border/80 bg-surface p-6 shadow-[0_20px_60px_rgba(79,70,229,0.08)] dark:border-border/40 dark:bg-surface dark:shadow-[0_20px_60px_rgba(99,102,241,0.18)] sm:p-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {techList.map((tech, index) => (
          <div
            key={tech.name}
            className={`interactive-lift motion-rise-in motion-delay-${(index % 4) + 1} flex items-start gap-4 rounded-3xl border border-border/70 bg-background/80 p-4 shadow-sm hover:border-primary/60 hover:bg-surface-muted dark:bg-surface dark:hover:bg-surface-muted`}
          >
            <div className="flex h-14 w-14 shrink-0 items-center justify-center gap-0.5 rounded-2xl border border-border/70 bg-white p-2 shadow-sm dark:bg-slate-100">
              {tech.icons.map((icon) => (
                <svg
                  key={icon.slug}
                  viewBox="0 0 24 24"
            ...
            <div>
              ...
              <p className="text-sm leading-6 text-muted">{tech.description}</p>
            </div>
          </div>
        ))}
      </div>
